...
/*
  ...
  2. 分类筛选
  3. 球员对比
  4. 比赛记录本地保存 localStorage
  5. 数据看板升级
*/

const STORAGE_KEY = "football_tactics_center_v4_players";
...
function getAttributeAverage(player) {
  const values = Object.values(player.attributes || {});
  if (!values.length) return 0;
  return Math.round(values.reduce((sum, value) => sum + value, 0) / values.length);
}

function getTopPlayer(players, key) {
...
function loadPlayers() {
  try {
    ...
    return Array.isArray(parsed) && parsed.length ? parsed : initialPlayers;
  } catch {
...
export default function App() {
  ...
  const filteredPlayers = useMemo(() => {
    return players.filter((p) => {
      ...
      const matchSearch =
        p.name.includes(search) || p.position.includes(search);
  ...
  const teamStats = useMemo(() => {
    ...
    return {
      ...
      bestPlayer: getTopPlayer(players, "ability"),
    };
  }, [players]);
    ...
    <div className="fm-shell">
      ...
      <aside className="fm-left-nav">
        ...
        {[
          ...
          { id: "data", label: "数据统计" },
          { id: "compare", label: "球员对比 ⭐" },
        ].map((nav) => (
          <button
            key={nav.id}
            className={view === nav.id ? "nav-item active" : "nav-item"}
        ...
        {view === "dashboard" && (
          <section className="dashboard-grid">
            ...
            <div className="panel wide">
              <h2>当前最强球员</h2>
              {teamStats.bestPlayer && (
                <p>
                  {teamStats.bestPlayer.name}（能力 {teamStats.bestPlayer.ability}）
                </p>
              )}
            </div>
          </section>
        ...
        {view === "players" && (
          <div className="player-layout">
            <aside className="player-list">
              <div className="side-title">球员筛选</div>
              ...
              <div className="side-title">球员列表</div>
        ...
        {view === "compare" && (
          <section className="compare-page">
            <div className="panel wide">
              <h2>球员对比</h2>
              <p className="muted">
                选择两名球员，对比能力、潜力、属性与角色定位。
              </p>

              <div className="compare-selects">
                <select
                  value={compareNames[0] || ""}
                  onChange={(e) =>
                    setCompareNames([e.target.value, compareNames[1] || ""])
                  }
                >
                  <option value="">选择球员 A</option>
                  {players.map((player) => (
                ...
                <select
                  value={compareNames[1] || ""}
                  onChange={(e) =>
                    setCompareNames([compareNames[0] || "", e.target.value])
                  }
                >
                  <option value="">选择球员 B</option>
                  {players.map((player) => (
                    <option key={player.name} value={player.name}>
                      {player.name}
                    </option>
            ...
            {compareNames[0] && compareNames[1] && (
              <ComparePanel
                playerA={players.find((p) => p.name === compareNames[0])}
                playerB={players.find((p) => p.name === compareNames[1])}
              />
            )}
          </section>
        )}
...
function ComparePanel({ playerA, playerB }) {
  ...
  return (
    <div className="compare-grid">
      <CompareCard player={playerA} />
      <CompareCard player={playerB} />

      <section className="panel wide">
        <h3>属性对比</h3>

        {attributeKeys.map((key) => {
          const a = playerA.attributes[key] || 0;
          const b = playerB.attributes[key] || 0;
...
function CompareCard({ player }) {
  return (
    <section className="panel compare-card">
      <div className="number-card small">
        <span>{player.number}</span>
      </div>
      ...
      <div className="basic-table">
        ...
        <div>
          <span>平均属性</span>
          <b>{getAttributeAverage(player)}</b>
        </div>
        <div>
          <span>风格</span>
          <b>{player.style}</b>
